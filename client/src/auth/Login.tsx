...
function tryFromRecord(obj: Record<string, unknown> | undefined): string | undefined {
...
const Login: React.FC = () => {
  ...
  const onSubmit = async (data: LoginFormData) => {
    let res: AuthResult;
    try {
      res = await loginApi(data.email, data.password);
    } catch {
  ...
  return (
    <div className='flex flex-col justify-center gap-24 container mx-auto px-11 lg:px-72 py-8'>
      ...
      <main className='flex flex-col h-scren mt'>
        ...
        <div className='flex flex-col lg:items-center lg:flex-row  gap-x-40 mt-7'>
          <form onSubmit={handleSubmit(onSubmit)} className='w-full lg:w-[60%] max-w-sm space-y-4'>
            ...
            <div className='relative border rounded-lg px-3 pt-5 pb-2 h-16'>
              <label className='absolute top-1.5 left-3 text-xs'>Password</label>
              <input
                type='password'
